...
import numpy as np
try:
    import pandas_ta as pta  # si eventualmente lo volvés a incluir
except Exception:
    from indicators.pta_shim import PandasTAShim as pta
...
def compute_indicators(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    if sup and sup.get("enabled", False):
        st = ta.supertrend(high, low, close, length=sup.get(
            "length", 10), multiplier=sup.get("multiplier", 3.0))
        _add(df, "supertrend", st["SUPERT_10_3.0"]
             if "SUPERT_10_3.0" in st else st.iloc[:, 0])
    ...
    for n in trend.get("donchian", []):
        ...
        _add(df, f"donch_mid_{n}",
             don["DCL_{}_{}".format(n, n)].rolling(1).mean())
    ...
    for n in mom.get("stoch", []):
        ...
        _add(df, f"stoch_k_{n}", st["STOCHk_{}_3_3".format(
            n)] if f"STOCHk_{n}_3_3" in st else st.iloc[:, 0])
        _add(df, f"stoch_d_{n}", st["STOCHd_{}_3_3".format(
            n)] if f"STOCHd_{n}_3_3" in st else st.iloc[:, 1])
    for n in mom.get("stochrsi", []):
        _add(df, f"stochrsi_{n}", ta.stochrsi(close, length=n))
    macd_cfg = mom.get("macd", {"fast": 12, "slow": 26, "signal": 9})
    if macd_cfg:
        macd = ta.macd(
            close, fast=macd_cfg["fast"], slow=macd_cfg["slow"], signal=macd_cfg["signal"])
        _add(df, "macd", macd["MACD_12_26_9"]
             if "MACD_12_26_9" in macd else macd.iloc[:, 0])
    ...
    for n in vol.get("bollinger", []):
        ...
        _add(df, f"bb_high_{n}", bb["BBU_{}".format(n)]
             if f"BBU_{n}" in bb else bb.iloc[:, 0])
        _add(df, f"bb_low_{n}",  bb["BBL_{}".format(n)]
             if f"BBL_{n}" in bb else bb.iloc[:, 2])
        _add(df, f"bb_mid_{n}",  bb["BBM_{}".format(n)]
             if f"BBM_{n}" in bb else bb.iloc[:, 1])
    ...
    for n in vol.get("donch_width", []):
        ...
        width = (dc.iloc[:, 1] - dc.iloc[:, 0]) / \
            (dc.iloc[:, 2].rolling(n).mean() + 1e-9)
    ...
    if patt.get("heikin_ashi", False):
        ...
        _add(df, "ha_close", ha["HA_close"]
             if "HA_close" in ha else ha.iloc[:, 3])
    ...
    if patt.get("pivots", False):
        ...
        _add(df, "pivot_s1", pp.filter(
            like="S1").iloc[:, 0] if not pp.empty else pd.Series(np.nan, index=df.index))
        _add(df, "pivot_r1", pp.filter(
            like="R1").iloc[:, 0] if not pp.empty else pd.Series(np.nan, index=df.index))
    ...
    for n in sml.get("rolling_stats", []):
        ...
        df[f"roll_skew_{n}"] = close.rolling(n).apply(
            lambda x: pd.Series(x).skew(), raw=False)
        df[f"roll_kurt_{n}"] = close.rolling(n).apply(
            lambda x: pd.Series(x).kurt(), raw=False)
    ...
    for l in sml.get("autocorr_lags", []):
        df[f"autocorr_{l}"] = close.pct_change().rolling(
            100).apply(lambda x: pd.Series(x).autocorr(lag=l))
    ...
    for n in sml.get("percentile_range", []):
        ...
        df[f"pct_in_range_{n}"] = (close - roll_min) / \
            (roll_max - roll_min + 1e-9)
